...
@section('page_title')
	Gallery Album
	<span>
		<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/dashboard')}}">Dashboard</a> / <span>Gallery Album</span>
	</span>
@endsection
...
@section('help')
	<ul class="menu-help-list-container">
		...
		<li>
			Gunakan tombol View di dalam tombol Action untuk melihat detail dari Gallery album
		</li>
		<li>
			Gunakan tombol Edit di dalam tombol Action untuk mengedit Gallery album
		</li>
		<li>
			Gunakan tombol Delete di dalam tombol Action untuk menghapus Gallery album
		</li>
	</ul>
@endsection
...
@section('search')
	{!!Form::open(['URL' => URL::current(), 'method' => 'GET'])!!}
		<div class="menu-group">
			...
			<div class="menu-search-group">
				{!!Form::label('src_gallerycategory_id', 'Category', ['class'=>'menu-search-label'])!!}	
				{!!Form::select('src_gallerycategory_id', $category_options, null, ['class'=>'menu-search-text select'])!!}
			</div>
			<div class="menu-search-group">
		...
		<div class="page-item col-1">
			<div class="page-item-content">
				<div class="index-desc-container">
					<a class="index-desc-item" href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/gallery-album/create')}}">
						...
						<tr>
							...
							<td>
								<div class="index-order-group">
									{!!Form::open(['url' => URL::to(Crypt::decrypt($setting->admin_url) . '/gallery-album/moveto'), 'class'=>'index-form'])!!}

										{!!Form::hidden('id', $galleryalbum->id)!!}
										{!!Form::text('moveto', $galleryalbum->order, ['class'=>'index-order-text'])!!}
									...
									@if ($records_count > 1)
										@if ($counter == 1)
											{!!HTML::link(URL::to(Crypt::decrypt($setting->admin_url) . '/gallery-album/movedown/' . $galleryalbum->id), '', ['class'=>'index-form-down'])!!}
										@endif
										
										@if (($counter != 1) AND ($counter != $records_count))
											{!!HTML::link(URL::to(Crypt::decrypt($setting->admin_url) . '/gallery-album/moveup/' . $galleryalbum->id), '', ['class'=>'index-form-up'])!!} 
											{!!HTML::link(URL::to(Crypt::decrypt($setting->admin_url) . '/gallery-album/movedown/' . $galleryalbum->id), '', ['class'=>'index-form-down'])!!}
										@endif
							...
							<td>
								{!!$galleryalbum->is_active == true ? "<span class='text-green'>Active</span>":"<span class='text-red'>Not Active</span>"!!}
							</td>
							<td class="index-td-icon">
								<div class="index-action-switch">
									...
									<ul class="index-action-child-container" style="width: 110px">
										<li>
											<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/gallery-album/' . $galleryalbum->id)}}">
										...
										<li>
											<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/gallery-album/' . $galleryalbum->id . '/edit')}}">
									...
									<div class="index-del-content">
										<div class="index-del-title index-del-item">
											Do you really want to delete this gallery album?<br>
											All data related with this album will be delete too.
										</div>
										...
										{!!Form::open(['url' => URL::to(Crypt::decrypt($setting->admin_url) . '/gallery-album/' . $galleryalbum->id), 'method' => 'DELETE', 'class'=>'form index-del-item'])!!}
				...
				{{-- 
					Pagination
				 --}}
				{{$galleryalbums->appends($criteria)->links()}}
			</div>
		</div>
	</div>
@endsection