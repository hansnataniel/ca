...
@section('page_title')
	News
	<span>
		<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/dashboard')}}">Dashboard</a> / <span>News</span>
	</span>
@endsection
...
@section('help')
	<ul class="menu-help-list-container">
		...
		<li>
			Gunakan tombol View di dalam tombol Action untuk melihat detail dari News
		</li>
		<li>
			Gunakan tombol Edit di dalam tombol Action untuk mengedit News
		</li>
		<li>
			Gunakan tombol Delete di dalam tombol Action untuk menghapus News
		</li>
		<li>
			Gunakan tombol Save as Newsletter di dalam tombol Action untuk menyimpan News sebagai Newsletter
		</li>
		<li>
			Gunakan tombol Save & Broadcast as Newsletter di dalam tombol Action untuk menyimpan News sebagai Newsletter sekaligus membroadcastnya ke Newsletter subscriber
		</li>
	</ul>
@endsection
		...
		<div class="page-item col-1">
			<div class="page-item-content">
				<div class="index-desc-container">
					<a class="index-desc-item" href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/news/create')}}">
				...
				<table class="index-table">
					...
					@foreach ($newss as $news)
						...
						<tr>
							...
							<td>
								{{Str::words($news->short_desc, 20)}}
							</td>
							<td>
								{!!$news->is_active == true ? "<span class='text-green'>Active</span>":"<span class='text-red'>Not Active</span>"!!}
							</td>
							<td class="index-td-icon">
								<div class="index-action-switch">
									...
									<ul class="index-action-child-container" style="width: 200px">
										<li>
											<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/news/newsletter/' . $news->id)}}">
												{!!HTML::image('img/admin/index/newsletter_icon.png')!!}
												<span>
													Save as Newsletter
												</span>
											</a>
										...
										<li>
											<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/news/broadcast/' . $news->id)}}" class="separator">
												{!!HTML::image('img/admin/index/broadcast_icon.png')!!}
												<span>
													Save & Broadcast as Newsletter
												</span>
											</a>
										</li>
										<li>
											<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/news/' . $news->id)}}">
										...
										<li>
											<a href="{{URL::to(Crypt::decrypt($setting->admin_url) . '/news/' . $news->id . '/edit')}}">
									...
									<div class="index-del-content">
										...
										{!!Form::open(['url' => URL::to(Crypt::decrypt($setting->admin_url) . '/news/' . $news->id), 'method' => 'DELETE', 'class'=>'form index-del-item'])!!}
